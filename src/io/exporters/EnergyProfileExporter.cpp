...
ExportResult EnergyProfileExporter::exportSlabDose(
    ...
    const std::string& output_path) const
{
    ...
    if (!out) {
        result.success = false;
        result.error = ioError("No se pudo crear slab_dose.csv", output_path);
        return result;
    }

    out << "# BeamLabStudio tissue slab dose summary\n";
    out << "slab_id,material,density_g_cm3,axial_start_m,thickness_m,"
           "total_edep_MeV,total_edep_eV,mean_dose_Gy,tracks_crossing\n";
    ...
    for (const auto& slab : slabs) {
        ...
        const double mean_dose = crossings > 0
            ? total_edep / static_cast<double>(crossings) * 1.602176634e-13
              / (slab.material.density_g_cm3 * 1000.0 * slab.thickness_m * 1.0e-4)
            : 0.0;

        out << std::scientific << std::setprecision(9)
            << slab.id << ','
            << slab.material.name << ','
            << slab.material.density_g_cm3 << ','
            << slab.axial_start_m << ','
            << slab.thickness_m << ','
            << total_edep << ','
            << total_edep * 1.0e6 << ','
